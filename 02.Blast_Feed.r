...
# Check if there are incomplete command lines
if(length(args) < 2) {
...
download_ncbi_sequence <- function(accession, output_file) {
  url <- paste0("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&id=", accession,
  ...
  error = function(e) {
    if (grepl("HTTP status was '502'", conditionMessage(e))) {
      ...
      Sys.sleep(60)
      download_ncbi_sequence(accession, output_file)
    } else {
      cat("\nError downloading sequence: ", accession, "\n")
